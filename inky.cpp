...
#include"inky.h"
inky::inky(int a, int b) :
		ghost(a, b) {
}
void inky::getghostposition(int &a, int &b) {
	a = x;
	b = y;
}
void inky::setghostposition(int a, int b) {
	x = a;
	y = b;
}
...
void inky::draw() {
	...
	a = 12 - a;
	ColorNames lcolor,ecolor,body=BLUE_VIOLET;
	if(mode<21)
	{
    	lcolor=WHITE;
	    ecolor=RED;
...
int inky::search_corners()
{
	deque<Point> dq_points;
		...
		getghostposition(a, b);
		Point present_point(a, b), corner_point(1,15);
