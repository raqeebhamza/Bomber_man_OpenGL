...
#include"blinky.h"
blinky::blinky(int a, int b) :
		ghost(a, b) {
}

void blinky::getghostposition(int &a, int &b) {
	a = x;
	b = y;
}
void blinky::setghostposition(int a, int b) {
	x = a;
	y = b;
}
blinky::~blinky() {
...
int blinky::search_corners()
{
	deque<Point> dq_points;
		set_B_arr();
		int a, b;
		getghostposition(a, b);
		cout<<"search_corners"<<endl;
		Point present_point(a, b), corner_point(1,1);
